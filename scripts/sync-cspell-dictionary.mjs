...
function parseCodebookWords(source) {
  ...
  while (index < source.length) {
    ...
    if (quote !== '"' && quote !== "'") {
      throw parseError(source, index, 'expected a TOML string or closing bracket');
    }
  ...
  for (let i = index + 1; i < source.length; i++) {
    ...
    if (char === '"') {
      try {
        ...
        throw parseError(source, index, `invalid TOML basic string: ${error.message}`);
      }
    }
  }
...
function validateWords(words) {
  ...
  for (const word of words) {
    ...
    if (/\s/.test(word)) {
      throw new Error(`cspell dictionary entries cannot contain whitespace: ${word}`);
    }
    if (seen.has(word)) {
